...
export const HomePage = () => {
  ...
  const handleSearch = (results: Poster[], term: string) => {
    ...
    setSearchTerm(term);
    // Ne pas réinitialiser la catégorie lors d'une recherche
    // La recherche se fait maintenant dans la catégorie sélectionnée
  };

  // Logique de filtrage combinée (recherche + catégorie)
  const filteredPosters = selectedCategory 
    ? (searchTerm.trim() 
        ? searchResults.filter(poster => poster.category_id === selectedCategory)  // Recherche dans la catégorie sélectionnée
        : posters.filter(poster => poster.category_id === selectedCategory))  // Afficher la catégorie sélectionnée
    : [];  // Aucune affiche si aucune catégorie sélectionnée
        ...
        <section id="gallery" className="py-12 sm:py-16 lg:py-20 bg-gray-50 dark:bg-gray-800">
          <Container>
              ...
              {(
                <>
                  <div className="flex flex-wrap justify-center gap-4 sm:gap-6 lg:gap-8 px-4 sm:px-0">
                    {/* Afficher d'abord toutes les catégories */}
                    ...
                    {/* Bouton "Toutes" en dernière position */}
                    <div
                      className={`flex flex-col items-center gap-2 sm:gap-3 cursor-pointer group transition-all duration-300 ${
            ...
            {loading ? (
              ...
              // Aucune catégorie sélectionnée - Message d'invitation
              <div className="text-center py-20">
                ...
                <p className="text-xl text-gray-600 dark:text-gray-400 mb-8 max-w-2xl mx-auto">
                  Sélectionnez une catégorie ci-dessus pour découvrir nos magnifiques affiches personnalisables
                </p>
                <div className="space-y-3 text-sm text-gray-500 dark:text-gray-400">
                  <p>✨ Plus de 150 modèles disponibles</p>
                  <p>🎯 Organisés par événements et fêtes juives</p>
                  <p>⚡ Personnalisation instantanée sur Canva</p>
                </div>
              </div>
            ) : searchTerm.trim() && filteredPosters.length === 0 ? (
              // Recherche active mais aucun résultat dans la catégorie
              <div className="text-center py-20">
                ...
                <p className="text-gray-500 dark:text-gray-500 mb-6">
                  Aucune affiche trouvée pour "{searchTerm}" dans cette catégorie
                </p>
                <div className="space-y-3 text-sm text-gray-400">
                  ...
                  <div className="flex flex-wrap justify-center gap-4 max-w-md mx-auto">
                    ...
                    <span className="bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full">Essayez une autre catégorie</span>
                  </div>
                </div>
                <div className="mt-8">
                  <Button 
              ...
              <>
                ...
                <div className="text-center mb-8">
                  <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                    {categories.find(cat => cat.id === selectedCategory)?.name || 'Catégorie sélectionnée'}
                  </h3>
                  <p className="text-gray-600 dark:text-gray-400">
                    {filteredPosters.length} affiche{filteredPosters.length > 1 ? 's' : ''} disponible{filteredPosters.length > 1 ? 's' : ''}
              ...
              // Catégorie sélectionnée mais aucune affiche
              <div className="text-center py-20">
                <div className="text-gray-400 mb-4">
                  <span className="text-6xl opacity-30">📂</span>
                </div>
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                  Aucune affiche dans cette catégorie
                </h3>
                <p className="text-gray-600 dark:text-gray-400 mb-6">
                  La catégorie "{categories.find(cat => cat.id === selectedCategory)?.name}" ne contient pas encore d'affiches.
                </p>
                <div className="mt-8">
                  <Button 
                    onClick={() => setSelectedCategory(null)}
