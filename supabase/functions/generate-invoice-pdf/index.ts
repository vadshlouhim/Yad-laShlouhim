...
async function generatePDFFromHTML(html: string): Promise<Uint8Array> {
  // Use htmlcsstoimage.com API for PDF generation
  // This is a reliable service for HTML to PDF conversion
  const apiKey = Deno.env.get('HTMLCSSTOIMAGE_API_KEY')
  
  if (!apiKey) {
    throw new Error('HTMLCSSTOIMAGE_API_KEY not configured')
  }
...
serve(async (req) => {
  ...
  try {
    const { sessionId, invoiceId, generateOnly = false } = await req.json()
    ...
    if (!invoice) {
      return new Response(
        JSON.stringify({ error: 'Facture non trouvée ou non créée' }),
        { 
          status: 404, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    // If only generating HTML for preview
    if (generateOnly) {
    ...
    if (invoice.pdf_url && invoice.pdf_generated_at) {
      const generatedDate = new Date(invoice.pdf_generated_at)
      const hoursSinceGeneration = (Date.now() - generatedDate.getTime()) / (1000 * 60 * 60)
      
      // If PDF was generated less than 24 hours ago, return existing URL
    ...
    try {
      ...
      const fileName = `${invoice.invoice_number.replace('/', '_')}.pdf`
      const pdfUrl = await uploadPDFToSupabase(supabase, pdfBuffer, fileName)
